...
→ **对我们（重要的诚实警告）**：我们 §4 **删掉了验收闸门**（在线没留出集）。但论文恰恰说闸门是**承重墙之一**。这意味着我们用来替代它的"下一批自纠 + 拒绝缓冲"**承担了关键重量，不能做弱**：
- 拒绝缓冲（rejected buffer）论文里和闸门并列承重 → 我们**必须照搬且做实**（[步6](./prompts/b-stage-prompts.md)），失败指纹比对要可靠。
- minibatch/batch 大小这些我们可以放心简化（量小直接整桶、跳过 aggregate），论文背书"对这些不敏感"。
- 慢更新承重 → 我们交给 Curator(C) 的保护区维护**不是可选项**，是补偿删掉闸门后的纵向稳定性。
...
→ **对我们**：①"聚合前先规范化、先看真实数据区"正是我们文章 §5 那个"SUM 范围误含表头行"失败指纹的同类问题，**论文实证了这类过程性修正确实涨分**；②说明 B 的产出应当是**这种过程性、可泛化的操作纪律**，而不是绑死某张表的硬编码——这就是 analyst prompt 里反复强调 generalizable 的实证依据。
...
| **rejected-edit 当负反馈回灌** | 被否过的改动（连同失败模式）统一塞进一个上下文通道，下次分析时一并给它看，告诉它"这些试过没用、别再提"。 | 让系统**记住自己的失败尝试**，不在同一个坑里反复栽——在线删了 gate 后，这条尤其承重。 | W 注入 analyst / A 注入 agent prompt｜`reflect.py` 的 `step_buffer_context` |
...
|---|---|---|---|
| ⭐**验收 gate** | 每改一版 skill，就在专门的留出集上重跑打分，**分数严格更高才接受**，否则撤销、改动进拒绝缓冲。 | 要"同一批题可反复重放打分"的留出集，在线只有真实用户的一次性使用，**没有可重放留出集**。而论文 §4 说 gate 是**承重墙之一** → 替代物必须做实，是 **B 头号风险点**。 | **下批失败指纹自纠 + rejected_edits**（[§4](#4)）｜`gate.py`：`cand_score > current_score` |
